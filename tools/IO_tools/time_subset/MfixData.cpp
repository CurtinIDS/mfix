...
void MfixData::Time_subset_SPx(float t0 , float t1)
{
    for (int i=0; i<nspx_use; ++i)
     {
        int nvars = 0;
	...
	string fname_subset = run_name + "_SUBSET.SP" + ext[i];
	    ...
	    cout << "Processing file : " << fname << "\n";
            ...
            if (nvars > 0) 
            {
                FILE_POSITION offset = (FILE_POSITION)(512-sizeof(float)) + 
		                       (FILE_POSITION)512 * 
				       (FILE_POSITION)(nvars)*(FILE_POSITION)spx_records_per_timestep;
                ReadWriteValues( in , offset , i , nvars , fname_subset , t0 , t1);
            }
        }
    }
}
...
void MfixData::ReadWriteValues(ifstream & in , FILE_POSITION offset , int spxNum , int nvars ,
             const string & fname , float t0 , float t1)
{
	...
	++c;
        ...
        if ( time >= t0  &&  time<=t1)
        {
           ...
           cout << "\tprocessing time = " << setw(15) << time << "   (record # " << c << ")\n";

           SWAP_FLOAT(time);
	   
           out.write( (char*)(&time),sizeof(float) );
           out.write( buf , 512-sizeof(float) );
	...
	{
	   FILE_POSITION skip = (FILE_POSITION)nvars *
    ...
    cout << "\t" << fname << " : updating record 3 ... setting number of records = " << rec << "\n";
    fstream outb(fname.c_str(),ios::binary|ios::in|ios::out);
    outb.seekp(512*2,ios::beg);
    SWAP_INT(rec);
    outb.write( (char*)(&rec) , sizeof(rec));
    outb.close();
}
