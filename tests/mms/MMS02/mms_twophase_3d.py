...
ug = diff(Fvecz, yt) - diff(Fvecy, zt)

vg = diff(Fvecx, zt) - diff(Fvecz, xt)

wg = diff(Fvecy, xt) - diff(Fvecx, yt)

# check divergence free
divug = diff(ug,xt)+diff(vg,yt)+diff(wg,zt)
print('\n Gas velocity field is divergence free:')
print(simplify(divug)==0)

# solid velocity variable
us = us0*sin((pi*(xt + yt + zt))/2.)**2

vs = vs0*cos((pi*(xt + yt + zt))/2.)**2
